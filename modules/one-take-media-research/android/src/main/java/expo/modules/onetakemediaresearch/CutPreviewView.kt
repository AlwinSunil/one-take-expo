...
@UnstableApi
class CutPreviewView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  ...
  private fun load() {
    ...
    try {
      ...
      next.setComposition(CompositionFactory.build(JSONArray(source))); next.prepare(); next.playWhenReady = wantsPlay
  ...
  fun seek(value: Double) { player?.seekTo((value.coerceAtLeast(0.0) * 1000).toLong()) }
  override fun onAttachedToWindow() { super.onAttachedToWindow(); load(); handler.post(ticker) }
  override fun onDetachedFromWindow() { handler.removeCallbacksAndMessages(null); surface.player = null; player?.release(); player = null; loadedSource = null; super.onDetachedFromWindow() }
}
